# Registers the HermesHindsightWatchdog Windows Task Scheduler task.
...
$TaskName   = "HermesHindsightWatchdog"
$ScriptPath = "$env:USERPROFILE\Documents\GitHub\Hermes-Argus\deploy\hindsight-watchdog.ps1"
...
# MultipleInstances=IgnoreNew (set below) prevents double-launch when already running.
...
Register-ScheduledTask `
    ...
    -Settings  $settings `
    -Principal $principal `
    -Description "Hindsight watchdog -- polls port 8888 every 30s, restarts via hindsight-start.ps1 on failure" | Out-Null
...
Write-Host "Log: $env:USERPROFILE\.hermes\logs\hindsight-watchdog.log"
